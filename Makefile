...
# Docker run command
###############################################################################################################3
DOCKER_RUN=time docker run --rm -e "PLATFORM_NAME=molecule-test-$(PR_SHA)" -e "GIT_SHA=$(PR_SHA)" -e "_DATE=$(_DATE)" -e "TERM=xterm" -w /tmp/ansible-role-kolide
...
DOCKER_RUN+= -v "$(CURRENT_DIR):/tmp/ansible-role-kolide:rw"
...
.PHONY: run
run:
	ansible-playbook -vvvvv -i "localhost," -c local playbook.yml  --extra-vars="bossjones__oh__my__zsh__user=$(WHOAMI)"

.PHONY: run-ubuntu
run-ubuntu:
	ansible-playbook -vvvvv --ask-become-pass -i "localhost," -c local playbook_ubuntu.yml --extra-vars="bossjones__kolide__user=$(WHOAMI)"

.PHONY: run-ubuntu-version-manager
run-ubuntu-version-manager:
	ansible-playbook -vvvvv --ask-become-pass -i "localhost," -c local playbook_ubuntu_version_manager.yml --extra-vars="bossjones__kolide__user=$(WHOAMI) boss__user=$(WHOAMI) boss__group=$(WHOAMI)"
